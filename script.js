// ==========================================
// RAVIE.IN - UNIVERSAL CORE ENGINE (v2.3 FIXED)
// ==========================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Legal Streams (Bottom Dock)
    const sidePrivacy = document.getElementById('side-privacy');
    const sideTerms = document.getElementById('side-terms');
    const streamPrivacy = document.getElementById('ledger-privacy');
    const streamTerms = document.getElementById('ledger-terms');
    const secondaryMenu = document.querySelector('.compass-secondary');

    // ==========================================
    // 1. UI OVERLAY & HAMBURGER CONTROLS
    // ==========================================
    ...
    function switchStream(target) {
        ...
        if (target === 'talents') {
            ...
            targetDepth = 0.35; // Calibrated depth shift
    ...
    // ==========================================
    // 3. LEGAL BOTTOM DOCK & TASKBAR ROUTER
    // ==========================================
    function switchLegalStream(target) {
        if (!sidePrivacy || !sideTerms) return;

        sidePrivacy.classList.remove('active-sidebar');
        sideTerms.classList.remove('active-sidebar');
        if (streamPrivacy) streamPrivacy.classList.remove('active-stream');
        if (streamTerms) streamTerms.classList.remove('active-stream');

        if (target === 'privacy') {
            sidePrivacy.classList.add('active-sidebar');
            if (streamPrivacy) streamPrivacy.classList.add('active-stream');

            // Hide horizontal taskbar
            if (secondaryMenu) {
                secondaryMenu.classList.remove('submenu-open');
                secondaryMenu.classList.add('submenu-closed');
            }

            history.replaceState({ legal: 'privacy' }, '', '?doc=privacy');
            targetDepth = 0; 
            if (canvas) canvas.className = 'depth-surface';

        } else if (target === 'terms') {
            sideTerms.classList.add('active-sidebar');
            if (streamTerms) streamTerms.classList.add('active-stream');

            // Trigger horizontal slide-up taskbar
            if (secondaryMenu) {
                secondaryMenu.classList.remove('submenu-closed');
                secondaryMenu.classList.add('submenu-open');
            }

            history.replaceState({ legal: 'terms' }, '', '?doc=terms');
            targetDepth = 0.25; 
            if (canvas) canvas.className = 'depth-deep';
        }
    }

    if (sidePrivacy) sidePrivacy.addEventListener('click', () => switchLegalStream('privacy'));
    if (sideTerms) sideTerms.addEventListener('click', () => switchLegalStream('terms'));

    // ==========================================
    // 4. BROWSER HISTORY & DEEP LINK INTERCEPTORS
    // ==========================================
    window.addEventListener('popstate', (event) => {
        if (event.state && event.state.view) {
            transitionToPlatform(event.state.view, false);
        } else if (event.state && event.state.legal) {
            switchLegalStream(event.state.legal);
        } else {
            returnToGateway();
        }
    });

    // Homepage Direct Load (e.g., ravie.in/?view=brands)
    const urlView = new URLSearchParams(window.location.search).get('view');
    if (urlView === 'talents' || urlView === 'brands') {
        transitionToPlatform(urlView, false);
    }

    // Legal Page Direct Load (e.g., ravie.in/policy.html?doc=terms)
    const urlDoc = new URLSearchParams(window.location.search).get('doc');
    if (urlDoc === 'terms') {
        switchLegalStream('terms');
    }

    // ==========================================
    // 5. WEBGL FLUID ENGINE (THROTTLED & DEBOUNCED)
    // ==========================================
    if (canvas) {
        ...
        // Debounced resize to stop mobile scrolling stutter
        let resizeTimer;
